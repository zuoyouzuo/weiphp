...
<?php if(!empty($normal_tips)): ?><p class="normal_tips"><b class="fa fa-info-circle"></b> <?php echo ($normal_tips); ?></p><?php endif; ?>
      <div class="table-bar">
        <div class="fl">
          <div class="tools"><a href="javascript:void(0);" class="btn setting_tag">设置标签</a> &nbsp;<a href="javascript:void(0);" class="btn remove_tag">取消标签</a>&nbsp;<a href="<?php echo U ( 'Home/AuthGroup/export' );?>" class="btn">导出用户</a> &nbsp; 
            ...
            <tbody>
              <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                  ...
                  <a href="<?php echo U('Home/CreditData/lists',array('uid'=>$vo[uid]));?>" target="_self">积分记录</a>
                   <a href="javascript:void(0);" class="setting_tag" valid="ids%5B%5D=<?php echo ($vo["uid"]); ?>">设置标签</a>
                    <a href="javascript:void(0);" class="remove_tag" valid="ids%5B%5D=<?php echo ($vo["uid"]); ?>">取消标签</a>
    	...
    	<div class="manage_group normal_dialog">
            ...
            <select name="group" id="select_group" style="width:100%">
                <?php if(is_array($auth_group)): $i = 0; $__LIST__ = $auth_group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                          ...
                          <?php if(is_array($auth_group)): $i = 0; $__LIST__ = $auth_group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="check-item"><input type="checkbox" class="regular-checkbox select_tag" value="<?php echo ($vo["id"]); ?>" id="group_<?php echo ($vo["id"]); ?>" name="tags[]">
                             <label for="tag_<?php echo ($vo["id"]); ?>"></label><?php echo ($vo["title"]); ?> </div><?php endforeach; endif; else: echo "" ;endif; ?>'                  
                 

                <div class="btn_wrap"><button class="btn setting_tag" url="<?php echo U('changeGroup');?>" target-form="ids">确定</button></div>
            </div>
        </div>
    </div> -->
    <!-- 备注用户名 -->
...
//	})
    ...
    //用户标签弹框
		var html = $('<div class="manage_group normal_dialog">'+
                  '<div class="content">'+
                          '<?php if(is_array($auth_group)): $i = 0; $__LIST__ = $auth_group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>'+
                  ...
                  '<div class="content">'+
                          '<?php if(is_array($auth_group)): $i = 0; $__LIST__ = $auth_group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>'+
                            '<div class="check-item"><input type="checkbox" class="regular-checkbox select_tag" value="<?php echo ($vo["id"]); ?>" id="tag_<?php echo ($vo["id"]); ?>" name="tags[]">'+
                              '<label for="tag_<?php echo ($vo["id"]); ?>"></label><?php echo ($vo["title"]); ?> </div>'+
                          '<?php endforeach; endif; else: echo "" ;endif; ?>'+                  
                  '<div class="btn_wrap"><button class="btn setting_tag" url="<?php echo U('removeTag');?>" target-form="ids">确定</button></div>'+
                  '</div>'+
              '</div>');
    query = $('.ids').serialize();
    if(query==""){

...
2、对于认证服务号虽然使用高级群发接口的每日调用限制为100次，但是用户每月只能接收4条，无论在公众平台网站上，还是使用接口群发，用户每月只能接收4条群发消息，多于4条的群发将对该用户发送失败；' );
	...
	function _send_by_group($group_id) {
		...
		// $param ['msgtype'] = 'news';
		  if (I ( 'timed', 0, 'intval' )) { 
                        if($timed_date = I ( 'timed_date',0)){
                               // $file = "/alidata/www/weiphp/Application/Common/Cron/sss.log";
                            $path = APP_PATH.'Common/Cron/';
                            $name = date('ymd',strtotime($timed_date)).'.json';
                            $data['type'] = 'tag'; 
                            $data['url'] = $url;
                           // $data['conent'] = json_decode($paramStr,TRUE );
                               $data['conent'] = $paramStr;           
                                file_put_contents($path.$name,'@@'.urldecode(json_encode($data)),FILE_APPEND);
                                return 10086;
                        }else{
                            $this->error ( '开启定时，日期不能为空' );
                        }                           
                    }
...
// 		$this->assign ( 'noraml_tips', '当用户发消息给认证公众号时，管理员可以在48小时内给用户回复信息' );
	...
	function custom_sendall_lists() {
		...
		$nav [] = $res;
                ...
                $res ['title'] = '定时群发消息';
		$res ['url'] = U ( 'timed_sendall_lists',$param );
		$res ['class'] = $act == 'timed_sendall_lists' ? 'current' : '';
		$nav [] = $res;
		$this->assign ( 'nav', $nav );
                $this->assign ( 'nav', $nav );
		$this->assign ( 'nav', $nav );
...
// 		$list = M ( 'custom_sendall' )->where ( $map )->order ( 'id desc' )->group ( 'diff' )->selectPage ();
	...
	function timed_sendall_lists() {
		...
		//                 $path = APP_PATH.'Common/Cron/';
//                 var_dump(scandir($path));exit;  
		$url = U ( 'timed_sendall_lists',$this->get_param );
		$this->assign ( 'searchUrl', $url );
		$this->assign ( 'noraml_tips', '当用户发消息给认证公众号时，管理员可以在48小时内给用户回复信息' );

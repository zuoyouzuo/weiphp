...
class QrCodeModel extends Model {
	...
	function add_qr_code($action_name = 'QR_SCENE', $addon = '', $aim_id = '', $extra_int = '', $extra_text = '') {
		...
		$code = $this->QrcodeCreate ( $data ['scene_id'], $data ['action_name'] );
               
		if (!is_array($code)) {
			return - 2; // 获取二维码失败
		}
                $data ['qr_code'] = $code['qr_code'];
	...
	private function QrcodeCreate($qrcodeID, $qrcodeType = 'QR_SCENE') {
		...
		if (@array_key_exists ( 'ticket', $tempArr ) && @array_key_exists ( 'url', $tempArr )) {
                        $data = array();
                        $data['qr_code'] = 'https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=' . $tempArr ['ticket'];
                        $data['qr_url'] = $tempArr ['url'];
			return $data;
		} else {

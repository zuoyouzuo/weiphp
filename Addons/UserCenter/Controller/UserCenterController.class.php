...
//                           
//                $param ['openid_list'] = array('oZNfujlyE2vEjkpCWygYW1tCvtBE');
//                $param ['tagid'] = 102;
//                $param = JSON ( $param );
//                $res = post_data ( $url, $param );
//                #INSERT IGNORE  INTO wp_auth_group_access VALUES (1,5);
//                #DELETE FROM wp_auth_group_access WHERE uid=1 and group_id = 5;
//                var_dump($res);exit;
                /*************************************************/
	...
	public function changeGroup() {
		...
		D ( 'Home/AuthGroup' )->move_group ( $uids, $tags );
		foreach ( $uids as $uid ) {
		...
		D ( 'Home/AuthGroup' )->remove_tag ( $uids, $tags );
		foreach ( $uids as $uid ) {
			D ( 'Common/User' )->getUserInfo ( $uid, true );
		}
		echo 1;
	...
	function syc_user() {
			...
			foreach ( $param ['user_list'] as $p ) {
				...
				if (empty ( $tempArr )) {
					...
					// addWeixinLog($tempArr,'syc_userlistscount5');
                                ...
                                if($tempArr ['tagid_list']){
                                    $tempArr ['tagid_list'] = implode(',',$tempArr ['tagid_list']);
                                }                                
		...
		foreach ( $data ['user_info_list'] as $u ) {
			if ($u ['subscribe'] == 0)
				continue;
                        if($u ['tagid_list']){
                            $u ['tagid_list'] = implode(',',$u ['tagid_list']);
                        }   
	...
	function syc_user_group() {
		...
		$list = M ( 'user' )->where ( $user_map )->field ( 'uid,tagid_list' )->select ();
		foreach ( $list as $vo ) {
//                    if($vo['tagid_list']){
//                        $taglist = explode(',',$vo['tagid_list']);
//                        foreach ($taglist as $value) {
//                            $userArr [$vo ['uid']] = $value;
//                        }
//                    }else{
//                        $userArr [$vo ['uid']] = 0;
//                    }	
                    $userArr [$vo ['uid']] = $vo ['tagid_list'];
		}
		...
		foreach ( $uids as $uid ) {
			...
			if (isset ( $access [$uid] ) && $new_groupid == $old_groupid)
				continue;
                       $taglist = explode(',',$new_groupid);
                       if($taglist){
                           foreach ($taglist as $value) {
                               $save ['group_id'] = intval ( $wechatArr [$value] );
                               if (isset ( $access [$uid] )) {
                                        $amap ['uid'] = $uid;
